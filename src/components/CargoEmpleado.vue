...
export default {
    ...
    data() {
        return {
            ...
            cargo: {},
            cargos: [],
            actualizar_cargo: false,
            accion: 'Insertar cargo',
    ...
    methods: {
        getItems() {
            let self = this;
            let urlEndPoint = ''
            urlEndPoint = self.URL_API + "api/v1/cargos/" + self.cantidad
            if (this.currentUrl != '') {
        ...
        save() {
            if (this.checks.length > 0) { // validación para realizar actualización masiva
                ...
                if (self.actualizar) {
                    urlEndPoint = self.URL_API + "api/v1/cargos/" + self.idItem
                } else {
                    urlEndPoint = self.URL_API + "api/v1/cargos"
                }
                let config = this.configHeader();
                axios
                    .post(urlEndPoint, self.cargo, config)
                    .then(function (result) {
