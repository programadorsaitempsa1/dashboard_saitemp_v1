<!-- <template>
    <div>
        <div class="container">
            <h2>Administrar Estados de Contratos</h2>
...
export default {
    ...
    data() {
        return {
            ...
            accion: 'Insertar estado contrato',
            estado: {},
            currentUrl: '',
    ...
    methods: {
        ...
        getItems() {
            let self = this;
            let urlEndPoint = ''
            urlEndPoint = self.URL_API + "api/v1/estadocontrato/" + self.cantidad
            if (this.currentUrl != '') {
        ...
        response(response) {
            ...
            this.accion = 'Insertar estado contrato'
            this.checks = []
        },
        save() {
            if (this.checks.length > 0) { // validación para realizar actualización masiva
                ...
                if (self.actualizar) {
                    urlEndPoint = self.URL_API + "api/v1/estadocontrato/" + self.idItem
                } else {
                    urlEndPoint = self.URL_API + "api/v1/estadocontrato"
                }
                let config = this.configHeader();
                axios
                    .post(urlEndPoint, self.estado, config)
                    .then(function (result) {
        ...
        clear() {
            ...
            this.accion = 'Editar estado contrato'
        },
        check(check) {
            this.checks = check
        },
